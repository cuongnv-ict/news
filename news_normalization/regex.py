...
class regex:
    ...
    def run_regex_training(self, data):
        ...
        s = self.detect_special_mark.sub(u'5', s)
        s = self.detect_special_mark2.sub(u'6', s)
        # s = self.detect_special_mark3.sub(u'7', s)
    ...
    def run_regex_predict(self, query):
        ...
        s = self.rm_except_chars.sub(u'', s)
        s = self.normalize_special_mark.sub(u' \g<special_mark> ', s)
        s, mark = self.replace(self.detect_special_mark, u'5', s)
...
if __name__ == '__main__':
    s = u'Chào thầy/cô.thưa thầy cô cho e hỏi:vì sao khán giả có địa vị khác nhau sẽ có sự phản ứng khác nhau đối với cùng một bài thuyết trình ?e cảm ơn thầy cô'
    x = s.replace(u'khác nhau', u'khác_nhau')
    print x
    r = regex()
    s = u'bền k56k năm'
